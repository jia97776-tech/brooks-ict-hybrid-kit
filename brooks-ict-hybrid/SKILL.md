...
**Version:** 2026-07-12i (屏前确认后市价优先 + 客观触发合同). Policy lives here; evidence and detailed mechanics live in references.
...
选定后禁止附送第二入口、反抽加仓或另一张备用单。市价成交后不再写 TTL/撤单价，也不临时新增收盘软退出；只报硬损、管理位、主目标和 runner 激活价。空单硬损公式固定为 `max(序列高点, 止损侧旧高/等高/POI上沿) + buffer`，多单镜像。

市价计划必须先做并展示止损算术：`anchor = 最外层止损侧价格`，`buffer = max(0.25×ATR, 点差垫)`，`SL = anchor ± buffer`。不得向结构内取整。市价成交后禁止再写“取消/作废/失效线”；那是 pending 生命周期，不是持仓管理。管理位只执行减仓；保本仍是确认结构外 + buffer，禁止直接推入场价。
...
1. **位置：**先取止损侧最外层锚。多单 `anchor=min(序列低点, 未完成等低/旧低/POI下沿)`，`SL=anchor-buffer`；空单 `anchor=max(序列高点, 未完成等高/旧高/POI上沿)`，`SL=anchor+buffer`。buffer 至少 `0.25×ATR`，非加密还要覆盖点差垫。
2. **距离：** `|entry − SL| ≥ 0.5×ATR(entry TF)`（白名单 M1 scalp 除外）。
3. **流动性：**止损侧若仍有未完成的等高/等低、旧高低或未回补 M15/H4 POI，SL 必须放到整层流动性外。做不到就缩仓或不做；扫荡尚未打印真实极值时，禁止预写紧损 fade 单。
...
- **重合不升级：**跨 TF 或单 TF OB+FVG 重合，只用于最外层止损、整层失效和目标 confluence；不得加仓、升评级或降低触发标准。证据见 `execution-aids-evidence.md`。
...
- **市价输出硬格式：**必须显示 `anchor / buffer / SL` 三个数；SL 不得向内取整。成交后不再写取消价、作废价或临时软退出。管理位只减仓，移损只到新确认结构外 + buffer，禁止把“推保本”写成移到入场价。
- 止损过三约束；不靠紧损凑 R，入场后不拓宽；第一止损触及即走。
