...
18. HYPE long trail-anchor dispute (07-12). Cycle: unknown intraday (post +1R manage on an intraday long). At decision time: entry 66.84 / initial SL 66.31 (1R=0.53); half banked at 67.37 (+1R, book locked +0.5R); remaining half to trail. One desk/user path: raise SL to **66.87** behind M5 double-tap lows **66.938/66.943** (bar ts 1783864200/1783864500; sequence then printed higher lows) + buffer. Competing path: raise toward **67.03** under the later equal-low shelf (bar ts 1783866000–1783867200, multiple M5 lows ~67.03–67.05). Call: user rejected shelf placement; desk converged on 66.87 only; 67.03 void. Outcome: policy write 2026-07-12k before full ticket close (manage rule, not P&L sample). Lesson: trail anchors = **confirmed structure that started the HL/LH sequence**, never the nearest **equal-low shelf** (first liquidity swept on normal pullback); multi-desk must align on the same anchor bar before quoting SL; slogan `移损跟确认结构，不跟最近货架`.

19. GBPUSD map-long limit vs user short + XAU early exit (07-13 London). Cycle: unknown intraday (GBP post-sweep bounce midrange after D1/H4 low 1.33664; XAU London spike-reject). At decision time: desk scan offered XAU short event plan and GBP **off-screen long map** zone 1.3375–1.3385 (revalidated today low 1.33664; refine 1.3378–1.3382; SL 1.33640; +1R scheme) while forbidding chase longs at ~1.340; user instead held GBP **short 1.3400** and earlier closed XAU short 4072→4076 (−0.36R) before hard 4083 / soft 4080.69. Call: manage the short as counter-structure near-only (SL 1.34140→trail wide 1.3403 after +1R half at 1.33860; main 1.33750; no runner; no long limit while short open); name XAU close as `fear_early_exit`. Outcome: GBP partial +1R executed; remain-half managed under trail; limit contract written into skill 2026-07-13a (not a P&L edge claim). Lesson: (a) deep limit is a full hangable contract (zone+SL+locked mgmt+cancel), never scanner paste; (b) open opposite ticket freezes the map limit — manage first; (c) +1R half is correct execution, discretionary scratch before stop/soft is a different journal flag.
